    ...
    if ret == True:
        ...
        if frameCnt > 60:
            ...
            degree = math.degrees(math.atan2(
                oldPos[1]-curPos[1], oldPos[0]-curPos[0]))
            if degree > 45 and degree < 135:
                dir = "top"
            elif (degree > 135 and degree <= 180) or (degree < -135 and degree >= -180):
                dir = "right"
        ...
        final_image = cv.circle(
            final_image, (int(x+w/2), int(y+h/2)), 10, (0, 0, 255), -1)

        mask = cv.line(mask, (ox, oy), (int(x+w/2),
                       int(y+h/2)), (0, 255, 0), 2)
        ox, oy = int(x+w/2), int(y+h/2)

        final_image = cv.add(final_image, mask)
        # Using cv2.putText() method
        final_image = cv.putText(final_image, 'position: x={} y={} degree={} direction={}'.format(int(x+w/2)-600, int(y+h/2), degree, dir), (50, 50), cv.FONT_HERSHEY_SIMPLEX,
